...
import { SmartMoneyStrategy } from '../strategies/intraday/SmartMoneyStrategy';
...
class StrategyEngine extends EventEmitter {
  ...
  start(): void {
    ...
    this.registerStrategy(new SmartMoneyStrategy());
  ...
  private async onCandleClose(candle: ICandle, depthMetrics: MarketDepthMetrics): Promise<void> {
    try {
      ...
      for (const [name, strategy] of this.strategies.entries()) {
        ...
        try {
          // Run strategy logic (can be async)
          const signal = await strategy.onCandle(candle, depthMetrics);

          if (signal) {
            // Filter signal by quality score
            if (!this.isSignalQualityAcceptable(signal)) {
              console.log(`[StrategyEngine] ❌ Signal REJECTED: Quality score too low (${signal.qualityScore}/100, min: ${this.minQualityScore})`);
              continue;
            }

            // Signal generated and passed quality filter!
  ...
  private async handleSignal(
    ...
    console.log(`\n${'='.repeat(60)}`);
    console.log(`🎯 HIGH-QUALITY SIGNAL by ${strategy.name}`);
    ...
    console.log(`Strategy Weight: ${signal.strategyWeight}x`);
    console.log(`Multi-TF Aligned: ${signal.isMultiTimeframeAligned ? '✅ YES' : '❌ NO'}`);
    ...
    try {
      ...
      const savedSignal = await SignalModel.create({
        ...
        strategyWeight: signal.strategyWeight,
        isMultiTimeframeAligned: signal.isMultiTimeframeAligned,
  ...
  /**
   * Check if signal quality is acceptable for execution
   *
   * Quality thresholds:
   * - Base signals: >= 60/100
   * - Multi-timeframe aligned signals: Can be slightly lower but still valuable
   *
   * This ensures only high-probability signals are executed
   *
   * @param signal - Signal to evaluate
   * @returns true if signal should be executed
   */
  private isSignalQualityAcceptable(signal: Signal): boolean {
    // Minimum quality threshold
    if (signal.qualityScore < this.minQualityScore) {
      return false;
    }

    // Multi-timeframe aligned signals are highly valued
    // Even if quality is moderate, multi-TF alignment is strong confirmation
    if (signal.isMultiTimeframeAligned) {
      console.log(`[StrategyEngine] ✅ Signal APPROVED: Multi-TF aligned (Quality: ${signal.qualityScore}/100)`);
      return true;
    }

    // For non-multi-TF signals, require higher quality
    if (signal.qualityScore >= 70) {
      console.log(`[StrategyEngine] ✅ Signal APPROVED: High quality (${signal.qualityScore}/100)`);
      return true;
    }

    // Moderate quality without multi-TF alignment
    console.log(`[StrategyEngine] ⚠️  Signal MARGINAL: Quality ${signal.qualityScore}/100, no multi-TF alignment`);
    return false;
  }
}
